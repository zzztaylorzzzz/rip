...
if not owner then script:Destroy() return end
local literallytheentirescript;
function literallytheentirescript(owner)
	local literallytheentirescriptp2 = coroutine.wrap(function()
	...
	if studio then
		primaryScript = script.vampireLocal:Clone()
...
local function loadGui()
	...
	return CircleProgressGui, vampireActivated
end
...
screen, vampActivatedGUI = loadGui()
...
local function blackout()
	...
	function primary()
		...
		blackoutcce:Destroy()
		pbar:Destroy()
		if progress < 1 then return else primCharge, primEnabled = 0, false end
	end
	function secondary()
	end
	function endPrimary()
...
animate()]], owner.Character)
	end
	primaryScript.Name = "vampireLocal"
...
for _,v in pairs(owner.PlayerGui:GetChildren()) do if v.Name == "DeathStatus" or v.Name == "CircleProgressGui" or v.Name == "VH3AbilityUI" then v:Destroy() end end
	...
	local function infectStart(char, speedM)
		print("infect anims")
	end
	...
	local function infect()
		function primary(player, action, target, value)
			...
			literallytheentirescript(players:GetPlayerFromCharacter(target))
	...
	literallytheentirescriptp2()
end

literallytheentirescript(owner)
